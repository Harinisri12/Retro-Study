  ...
  const fetchRecipes = async (ingredients: string[]) => {
    ...
    try {
      ...
      const recipePromises = response.data.map(async (recipe: any) => {
        ...
        console.log(detailedRecipe.data); // Debugging line to check API response
  
        return {
          ...recipe,
          healthScore: detailedRecipe.data.healthScore, // ✅ Fixed key
...
.button-container {
  ...
  {filteredRecipes.length > 0 && (
    <div>
      ...
      <h3 className="text-2xl font-semibold mb-4">Top 3 Healthiest Recipes:</h3>
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
        {[...filteredRecipes]
          ...
          .map((recipe) => (
            <div
              key={recipe.id}
              className="bg-green-100 border border-green-400 rounded-lg shadow-md hover:shadow-xl transition-shadow p-4 cursor-pointer"
              onClick={() => openRecipeInNewTab(recipe)} // ✅ Now opens in a new tab
            >
              <img
                src={recipe.image}
                alt={recipe.title}
              ...
              />
              <h4 className="font-semibold text-lg mb-2 text-green-800">{recipe.title}</h4>
              <p className="text-sm text-gray-600 mb-2">Health Score: <strong>{recipe.healthScore}</strong></p>
              <p className="text-sm text-gray-600">Calories: {recipe.nutrition.calories}</p>
              <p className="text-sm text-gray-600">Carbs: {recipe.nutrition.carbs}</p>
              <p className="text-sm text-gray-600">Protein: {recipe.nutrition.protein}</p>
            </div>
          ))}
      </div>

      {/* All Recipes */}
      <h3 className="text-2xl font-semibold mt-8 mb-4">All Recipes:</h3>
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
        {filteredRecipes.map((recipe) => (
          <div
            key={recipe.id}
            className="bg-white rounded-lg shadow-md hover:shadow-xl transition-shadow p-4 cursor-pointer"
            onClick={() => openRecipeInNewTab(recipe)} // ✅ Now consistent for all recipes
          >
            <img
              src={recipe.image}
              alt={recipe.title}
